...
<div class="jvcounter_contain jvcounter_<?php echo $moduleStyle;?>">
    ...
    <?php if($params->get('showip',1)){?>
        <div class="jvcounter_rows jvcounter_today">
            <?php
                ...
                echo '<span>'.JText::_('Your IP: ').'</span>'.$ip;
            ?>
        </div>
    <?php }?>
    ...
    <?php if($params->get('showdatetoday',1)){?>
        <div class="jvcounter_rows jvcounter_datetoday">
            <?php 
                ...
                echo  '<span>'.JText::_('Today: ').'</span>'.JFactory::getDate($timeoffset)->format($dateformat);
            ?>
        </div>
    <?php }?>    
    ...
    <?php if(isset($visits['online']) && $visits['online']){?>
        <div class="jvcounter_rows jvcounter_today">
            ...
            <span><?php echo JText::_('Thành viên trực tuyến :').' </span>'. $count['user'];?></span><br/>
            <span><?php echo JText::_('Khách trực tuyến:').' </span>'. $count['guest'];?></span><br/>
        </div>
    <?php }?>
    </div>
